-- Migration to ensure vehicle_assignment_devices junction table exists and is properly configured
-- This is a safe migration that will not affect existing data
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE table_name = 'vehicle_assignment_devices'
    ) THEN
        ...
        RAISE NOTICE 'Created vehicle_assignment_devices junction table';
    ELSE
        RAISE NOTICE 'vehicle_assignment_devices table already exists';
    END IF;
END $$;

-- Check if device_ids column still exists in vehicle_assignments table
-- If it exists, migrate data and drop the column
DO $$
DECLARE
    va_record RECORD;
    device_id_text TEXT;
    ...
    IF EXISTS (
        ...
        LOOP
            ...
            BEGIN
                ...
                IF device_ids_array IS NOT NULL THEN
